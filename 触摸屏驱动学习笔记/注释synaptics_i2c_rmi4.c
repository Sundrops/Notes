...
　//   赋值　指向自己写的函数，实现多态
	...
	* EV_SYN 0x00 同步事件
	...
	这些codes表明一个，两个，三个和四个手指参与触摸板和触摸屏的操作。例如，如果用户使用两只
	手指在触摸板上试图滚动屏幕上的内容，在运动期间，应该发送value为1的BTN_TOOL_DOUBLETAP。
	注意的是所有的BTN_TOOL_<name>codes 和 BTN_TOUCH code是用于正交目的的，一个触摸板由手
	 指碰触时，应该在这组中各生成一个事件c两ode，至少在一个同步帧中带有一个value为1的BTN_TOOL_<name>code。
	 * 
	*/
    ...
    // INPUT_PROP_DIRECT特性表明设备的坐标直接和屏幕坐标向对应（无需琐碎的转换操作，像缩放，反转，旋转等）。
	...
	if (gpio_is_valid(platform_data->irq_gpio)) {
		...
		/*
		gpio_set_value（port_num,0/1） 一般只是在这个GPIO口的寄存器上写上某个值，至于这个端口是否设置为输出，
		它就管不了！而gpio_direction_output （port_num,0/1)，在某个GPIO口写上某个值之后，还会把这个端口设置为输出模式。
		 因此，有人也许就会建议，把gpio_set_value这个函数直接去掉不用，是否可以，显然是可以的。
		 但是为什么系统还要用呢，
		 我个人分析是，
			系统开发人员在要结合这两者来使用，以便提高效率。
		   一般某个端口设置好了输入与输出模式后，最好不要经常变动。
		   首先要调用gpio_direction_output（），以后要设置高低电平时，直接使用gpio_set_value（）就可以了，这样可以省却
		   再次调用设置输出模式的操作，从而提高运行效率！
		 */
    ...
    */
	...
	// 在发生对应于第 1个参数 irq 的中断时，则调用第 2 个参数handler 为要注册的中断服务函数(也就是把 handler() 中断
	// 服务函数注册到内核中 )。
